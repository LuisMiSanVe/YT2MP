...
Para ello (en Windows 11), ve a Configuración > Sistema > Información > Configuración avanzada del sistema > Variables de entorno > buscar por `Path` y dale a `Editar...` > Nuevo > copia la ruta de la carpeta en la que está `ffmpeg.exe` y dale click a `Aceptar`.
